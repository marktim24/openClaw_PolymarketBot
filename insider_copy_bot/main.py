...
class DryRunCopyBot:
    ...
    def classify_trade(self, trade: Dict) -> str:
        ...
        if any(token in normalized for token in ['sell', 'close_sell', 'close', 'exit']):
            return 'SELL'
        if any(token in normalized for token in ['buy', 'purchase', 'open_buy']):
            return 'BUY_ENTRY'
        return 'UNKNOWN'
    ...
    def validate_entry(self, trade: Dict):
        ...
        try:
            entry_price = float(trade.get('price') or trade.get('avg_price') or trade.get('avgPrice') or 0)
        except Exception:
        ...
        if not market_id:
            market_id = trade.get('market_name') or trade.get('question') or trade.get('market') or ''
        if not market_id:
    ...
    def handle_csv_close_event(self, trade: Dict, trade_class: str):
        ...
        if not market_id:
            return
        try:
            exit_price = float(trade.get('price') or trade.get('avg_price') or trade.get('avgPrice') or 0)
        except Exception:
            return
        if exit_price <= 0:
            return
        closed_at = trade.get('timestamp') or utc_now_iso()
        closed = self.position_manager.close_oldest_by_market(market_id, exit_price, trade_class.lower(), closed_at)
        if closed:
    ...
    def process_trades(self, trades: List[Dict]):
        ...
        for trade in trades:
            market = trade.get('market_name') or trade.get('question') or trade.get('market') or trade.get('asset_id') or trade.get('condition_id') or 'UNKNOWN_MARKET'
            ...
            if valid:
                pos = self.position_manager.open_position(market_id, market, side, entry_price, opened_at)
